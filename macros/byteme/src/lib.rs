...
fn generate_fn_to_bytes(item: syn::Item) -> proc_macro2::TokenStream {
  match item {
...
fn expand_field(field: &syn::Field) -> proc_macro2::TokenStream {
  ...
  let fn_line = match field_type {
    syn::Type::Array(syn::TypeArray { elem, .. }) => parse_array(field_name, elem),
    syn::Type::Path(type_path, ..) if check_path(&type_path.path) => parse_path(field_name, type_path),
    syn::Type::Path(..) if attribute.is_some() => parse_path_with_attribute(field_name, attribute),
...
fn parse_array(field_name: &Option<syn::Ident>, elem: &Box<syn::Type>) -> proc_macro2::TokenStream {
  let ref_type = elem.as_ref();
  match ref_type {
    syn::Type::Path(type_path, ..) if check_type_path(type_path) => parse_array_path(field_name),
...
fn parse_path(field_name: &Option<syn::Ident>, field_type: &syn::TypePath) -> proc_macro2::TokenStream {
  quote! {
...
fn parse_path_with_attribute(
  ...
  attribute: Option<&syn::Attribute>,
...
fn parse_array_path(field_name: &Option<syn::Ident>) -> proc_macro2::TokenStream {
  quote! {
...
fn panic_for_field_type_missing_attribute(field_name: &Option<syn::Ident>, field_type: &syn::Type) -> ! {
